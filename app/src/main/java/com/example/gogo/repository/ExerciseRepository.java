...
public class ExerciseRepository {
    ...
    public ExerciseRepository(Context context) {
        ...
        planRepository = new ExercisePlanRepository(context);
    }
    ...
    public boolean deleteExercise(int id) {
        // Kiểm tra xem Exercise có đang được sử dụng trong ExercisePlan không
        if (planRepository.isExerciseInAnyPlan(id)) {
            return false; // Không xóa được, trả về false
        }

        SQLiteDatabase db = dbHelper.getWritableDatabase();
        int rowsAffected = db.delete("Exercise", "ExerciseID = ?", new String[]{String.valueOf(id)});
        dbHelper.close();
