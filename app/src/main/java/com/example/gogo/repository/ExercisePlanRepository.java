...
public class ExercisePlanRepository {
    ...
    public ExercisePlan getPlanByExerciseIdAndPlanName(int exerciseId, String planName, int userId) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery(
                "SELECT * FROM ExercisePlan WHERE ExerciseID = ? AND PlanName = ? AND UserID = ?",
                new String[]{String.valueOf(exerciseId), planName, String.valueOf(userId)}
        );
        if (cursor.moveToFirst()) {
    ...
    public int updatePlan(ExercisePlan plan) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put("UserID", plan.getUserID());
        values.put("ExerciseID", plan.getExerciseID());
    ...
    public boolean isExerciseInAnyPlan(int exerciseId) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT COUNT(*) FROM ExercisePlan WHERE ExerciseID = ?",
                new String[]{String.valueOf(exerciseId)});
        boolean isInUse = false;
        if (cursor.moveToFirst()) {
            isInUse = cursor.getInt(0) > 0;
        }
        cursor.close();
        dbHelper.close();
        return isInUse;
